...
#define N_GRAD 50  //Distanece for gardient calculation
...
public:
	...
	WaveAnalyser(float cutoff_freq = CUTOFF_FREQ, float sampling_time = SAMPLING_TIME, int order = INIT_ORDER, int n_data_array = N_DATA_ARRAY,
		int n_grad = N_GRAD, int innitial_calibration_delay = INNITAL_CALIBRATION_DELAY, int n_w = N_WAVES); //Constructor with defoult parameters
	void init(); //Initialisation
	void setup(); //Setup
	bool update(); //Update reading - call every time from the main loop

	//Set function
	void setCalibrationDelay(int); //Change calibration delay after initialisation
	void setNumberOfWaves(int); //Change number of waves to be measured after initialisation
